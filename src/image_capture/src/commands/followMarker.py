...
class FollowMarker:
    def __init__(self, robot, timer=False):
        ...
        print("Starting Follow Marker Algorithm...")
        ...
        self.distance = 300 # mm
        self.trans_ignore = 200
        self.rot_ignore = 80
        ...
        self.sub = rospy.Subscriber('/scan', LaserScan, self.takeAction)
        rospy.spin()
    ...
    def takeAction(self, msg):
        ...
        if depth != 0:
            if depth > self.distance:
                xm = self.moveBy
            elif depth < self.trans_ignore:
                xm = -self.moveBy

        # If it detected a marker, mp != 0
        if mp[1] < -self.rot_ignore:
            xr = -self.rotBy
        elif mp[1] > self.rot_ignore:
            xr = self.rotBy
...
if __name__ == "__main__":
    ...
    args.add_argument('--timer', default=False, type=str, help='do you want a timer of 30 seconds?')
